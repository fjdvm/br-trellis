    ...
    return (
      <main className="max-w-[1440px] mx-auto px-4 sm:px-8 py-12 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          <Skeleton className="lg:col-span-7 w-full aspect-[4/3] bg-surface-container" />
          <div className="lg:col-span-5 space-y-4">
            <Skeleton className="h-6 w-24 bg-surface-container" />
            <Skeleton className="h-10 w-3/4 bg-surface-container" />
            <Skeleton className="h-8 w-1/3 bg-surface-container" />
            <Skeleton className="h-24 w-full bg-surface-container" />
            <Skeleton className="h-12 w-48 bg-surface-container" />
          </div>
        </div>
      </main>
    );
  ...
  if (error || !product) {
    return (
      <main className="max-w-[1440px] mx-auto min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <Button asChild className="mt-6 bg-primary text-white hover:bg-primary-container font-semibold rounded-full px-8">
  ...
  return (
    <main className="max-w-[1440px] mx-auto px-4 sm:px-8 py-12 space-y-12 bg-surface">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 font-sans">
        <Link
          href="/products"
          className="inline-flex items-center gap-2 text-xs font-semibold text-primary hover:text-primary-container transition-all bg-surface-container-lowest hover:bg-surface-container-high px-5 py-2.5 border border-outline-variant/30 shadow-2xs active:scale-95"
        >
