...
export function ProductGrid({ products, loading }: ProductGridProps) {
      ...
      <div className="space-y-8 py-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="flex flex-col gap-4 p-4 border border-outline-variant/30 bg-surface-container-lowest rounded-2xl">
              <Skeleton className="w-full aspect-square rounded-xl bg-surface-container" />
  ...
  if (products.length === 0) {
    return (
      <div className="text-center py-20 px-4 bg-surface-container-low rounded-3xl border border-dashed border-outline-variant text-on-surface">
        <div className="mx-auto w-12 h-12 rounded-full bg-surface-container flex items-center justify-center mb-4 text-on-surface-variant">
